...
In order to compare blackbox model with an interpretable model a *decision tree classifier* was trained. It turned out that splits were made by features which are also important in blackbox model (*XGBoost*). More details on this are given below.
...
<!---
```{r ex_breakdown, echo=FALSE, fig.cap='Break down plot explaining prediction of chosen instance.', out.width = '50%', fig.align='center'}
...
<!---
```{r ex_lime, echo=FALSE, fig.cap='LIME plot explaining prediction of chosen instance.', out.width = '50%', fig.align='center'}
knitr::include_graphics("images/03-ex_lime.png")
```
-->

We can see that explanation of *XGBoost* model says that features chosen in the *decision tree* have also influence on prediction in *XGBoost* model. As illustrated in figure \@ref(fig:ex_cp) if chosen client had not canceled reservation in the past, they would be less likely to cancel this reservation. What is more, if the client had booked a hotel later, they would have known their plans better and it would decrease probability of cancellation. Maybe the client canceled booking because of big family event, accident or breaking up with a partner (booking for 2 adults). It is impossible to predict such events in advance.
...
[//]: # (TODO)
The model achieved AUC **0.967**, and the figure above presents its ROC curve.
